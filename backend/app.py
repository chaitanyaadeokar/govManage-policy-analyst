...
# .env is loaded by serve.py before any imports — no need to call load_dotenv() here.
# _BACKEND_DIR  = absolute path to backend/
# _FRONTEND_DIST = absolute path to frontend/dist/ (the production build)
...
def api_health():
    """API health-check — useful for uptime monitors and deployment probes."""
...
def serve_frontend(path):
    """Serve the React production build for all non-API routes (SPA catch-all)."""
    # Never intercept /api/* — let Flask's registered API routes handle them.
    ...
    # Fall back to index.html — React Router handles client-side navigation.
...
def trigger_event():
    ...
    if mode in {"advanced", "agentic"}:
        ...
        # 1. Drop JSON into 1_inbox  (absolute path — safe regardless of CWD)
        inbox_dir = os.path.join(_BACKEND_DIR, "agents_micro", "shared_queues", "1_inbox")
        ...
        complete_dir = os.path.join(_BACKEND_DIR, "agents_micro", "shared_queues", "7_complete")
